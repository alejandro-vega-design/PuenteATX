...
export default function CommunityPassportList({ session, lang, navigate }) {
  ...
  return <section className="community-page">
    ...
    {state === 'loading' && <p className="loading-inline" aria-live="polite"><span className="admin-button-spinner" aria-hidden="true"/><span>Loading…</span></p>}{state === 'error' && <div className="admin-error-state"><p>{t.retry}</p><button onClick={load}>{t.retry}</button></div>}
