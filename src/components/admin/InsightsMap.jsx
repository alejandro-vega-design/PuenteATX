...
const PADDING = 4;
const MAP_ZOOM = 1.16;
const DEFAULT_COUNTY = 'Travis';
// Travis keeps its own, more detailed boundary file; every other county has one
// generated by scripts/split-county-zip-maps.mjs.
const mapUrlFor = county => county === 'Travis' ? '/maps/austin-travis-zip-codes.geojson' : `/maps/insights-counties/${county.toLowerCase()}.geojson`;
...
export default function InsightsMap({ data, t }) {
  ...
  useEffect(() => {
    let active = true;
    setState('loading'); setGeojson(null); setActiveZip(null);
    fetch(mapUrlFor(county))
      .then(response => {
  ...
  // County picker sits on top of the map, top-left, in every state so the
  // viewer can always switch away from a county that is loading or has no data.
  const picker = <label className="insights-map-county"><span className="sr-only">{t.mapCounty}</span>
    <select value={county} onChange={event => setCounty(event.target.value)} aria-label={t.mapCounty}>{supportedCounties.map(name => <option key={name} value={name}>{name}</option>)}</select>
  </label>;
  if (state !== 'ready') return <div className="insights-map-content"><div className="insights-map-stage insights-map-stage-state">{picker}<div className="insights-map-state"><p>{state === 'loading' ? t.loading : state === 'missing' ? t.mapMissing : t.mapError}</p></div></div></div>;
  const zipsInCounty = new Set(shapes.map(shape => shape.zip));
  const countyAreas = (data?.visible || []).filter(area => zipsInCounty.has(area.area_code));
  const visible = new Map(countyAreas.map(area => [area.area_code, area]));
  // Share of the searches shown for THIS county (suppressed ZIPs are not in the response).
  const total = countyAreas.reduce((sum, area) => sum + Number(area.event_count || 0), 0);
  ...
  return <div className="insights-map-content">
    ...
    <p className="insights-panel-note">{visible.size ? t.mapNote : t.mapCountyEmpty.replace('{county}', county)}</p>
    ...
    {tableOpen && createPortal(<div className="insights-area-drawer-overlay" onMouseDown={event => event.target === event.currentTarget && setTableOpen(false)} onWheel={event => event.target === event.currentTarget && event.preventDefault()} onTouchMove={event => event.target === event.currentTarget && event.preventDefault()}>
      <aside ref={drawerRef} className="insights-area-drawer" role="dialog" aria-modal="true" aria-labelledby="insights-area-drawer-title">
        <header><h2 id="insights-area-drawer-title">{t.needsByArea} · {county}</h2><button ref={closeRef} type="button" onClick={() => setTableOpen(false)} aria-label={t.close}>×</button></header>
