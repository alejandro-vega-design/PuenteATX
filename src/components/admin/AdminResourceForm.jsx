...
export default function AdminResourceForm({ t, resource, resources = [], refresh, navigate, notify, canDeletePermanently = false, locationSearch = '' }) {
  const [form, setForm] = useState(resource ? { ...blank, ...resource } : blank); const [keywordInputs, setKeywordInputs] = useState({ es: (resource?.keywords_es || []).join(', '), en: (resource?.keywords_en || []).join(', ') }); const [slugEdited, setSlugEdited] = useState(Boolean(resource?.slug)); const [dirty, setDirty] = useState(false); const [message, setMessage] = useState(''); const [saving, setSaving] = useState(false); const [confirmArchive, setConfirmArchive] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false); const firstRef = useRef(null);
