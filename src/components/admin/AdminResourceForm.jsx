...
import { archiveResource, createResource, getAdminResources, publishResource, updateResource } from '../../data/repository';
...
export default function AdminResourceForm({ t, resource, resources = [], navigate, notify, locationSearch = '' }) {
  const [form, setForm] = useState(resource ? { ...blank, ...resource } : blank); const [keywordInputs, setKeywordInputs] = useState({ es: (resource?.keywords_es || []).join(', '), en: (resource?.keywords_en || []).join(', ') }); const [slugEdited, setSlugEdited] = useState(Boolean(resource?.slug)); const [dirty, setDirty] = useState(false); const [message, setMessage] = useState(''); const [saving, setSaving] = useState(false); const [confirmArchive, setConfirmArchive] = useState(false); const firstRef = useRef(null);
  ...
  return <><button className="admin-back-link" onClick={backToResources}><ChevronLeftIcon/>{t.backToResources}</button><header className="admin-page-header"><h1>{resource ? t.edit : t.addResource}</h1></header><form className={`admin-resource-form${resource ? ' has-sticky-actions' : ''}`} onSubmit={event => event.preventDefault()}>{message && <p className="admin-form-message" aria-live="polite">{message}</p>}
    ...
    <section><h2>{t.formContact}</h2><div className="admin-fields"><Field label={t.phone} name="phone" type="tel" inputMode="tel" autoComplete="tel" maxLength="12" value={form.phone} onChange={changePhone}/><Field label="SMS" name="sms_phone" type="tel" inputMode="tel" maxLength="12" value={form.sms_phone} onChange={changePhone}/><Field label="WhatsApp" name="whatsapp_phone" type="tel" inputMode="tel" maxLength="12" value={form.whatsapp_phone} onChange={changePhone}/><Field label="Email" name="email" type="email" value={form.email} onChange={change}/><Field label={t.website} name="website_url" type="url" value={form.website_url} onChange={change}/></div></section>
