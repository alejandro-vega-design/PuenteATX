...
export default function SavedListPage({ lang, t, resourceT, locationSearch, saved, onToggle, onClear, onImport, navigate }) {
  ...
  return <main className="saved-list-page print-root"><div className="site-container saved-list-container"><header><h1>{t.title}</h1>{displaySlugs.length > 0 && <p>{listIntro}</p>}</header>
    ...
    {loading ? <p className="loading-state"><span className="loading-inline"><span className="admin-button-spinner" aria-hidden="true"/><span>{resourceT.loading}</span></span></p> : displaySlugs.length === 0 ?<section className="public-state saved-empty"><h2>{t.emptyTitle}</h2><p>{t.emptyText}</p><button className="primary-button" onClick={() => navigate('/recursos')}>{t.browse}</button></section> : <>
