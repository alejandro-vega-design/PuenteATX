...
export const resourceCopy = {
  es: {
    resultsTitle: 'Recursos para ti', searchLabel: 'Buscar recursos', searchPlaceholder: 'Busca comida, salud, vivienda…', search: 'Buscar', filter: 'Filtrar', filters: 'Filtros', close: 'Cerrar', apply: 'Aplicar', clear: 'Quitar filtros', resultsFound: count => `${count} ${count === 1 ? 'recurso encontrado' : 'recursos encontrados'}`, resultsLoading: count => count ? `${count} recursos cargados…` : 'Cargando recursos…',
    ...
    save: 'Guardar recurso', savedState: 'Guardado', removeSaved: 'Quitar de Mi lista', details: 'Ver detalles', call: 'Llamar', openMap: 'Abrir dirección en Google Maps', website: 'Sitio oficial', source: 'Fuente', share: 'Compartir', shareThis: 'Compartir este recurso', printThis: 'Imprimir este recurso', report: 'Reportar información incorrecta', reportThis: 'Reportar este recurso', resourceOptions: 'Más opciones para este recurso', copied: 'Enlace copiado', shareError: 'No pudimos copiar el enlace.', closeNotification: 'Cerrar notificación', showMore: 'Ver más', showLess: 'Ver menos', whatsapp: 'WhatsApp', print: 'Imprimir', printed: 'Impreso el', verified: 'Verificado', location: 'Área', loadMore: count => `Mostrar más recursos · ${count} ${count === 1 ? 'restante' : 'restantes'}`, noResults: 'No encontramos recursos con esos filtros.', noResultsHelp: 'Prueba quitar un filtro o buscar otra palabra.', loadError: 'No pudimos cargar los recursos. Intenta nuevamente.', retry: 'Intentar nuevamente', loading: 'Cargando recursos…', loadingHelp: 'Estamos preparando el directorio. Esto puede tomar unos segundos.', loadingMore: 'Cargando más recursos…', demo: 'Los recursos mostrados son datos ficticios del prototipo.'
  },
  en: {
    resultsTitle: 'Resources for you', searchLabel: 'Search resources', searchPlaceholder: 'Search food, health, housing…', search: 'Search', filter: 'Filter', filters: 'Filters', close: 'Close', apply: 'Apply', clear: 'Clear filters', resultsFound: count => `${count} ${count === 1 ? 'resource found' : 'resources found'}`, resultsLoading: count => count ? `${count} resources loaded…` : 'Loading resources…',
...
Object.assign(adminCopy.es, {
  ...
  bulkArchiveConfirm: count => `¿Quieres archivar ${count} ${count === 1 ? 'recurso' : 'recursos'}?`, bulkArchived: count => `${count} ${count === 1 ? 'recurso archivado' : 'recursos archivados'}.`,
    bulkDeleteConfirm: count => `¿Eliminar ${count} ${count === 1 ? 'recurso' : 'recursos'}?`,
    bulkDeleteDescription: (count, published) => `Esta acción no se puede deshacer. ${count === 1 ? 'El recurso desaparecerá' : 'Los recursos desaparecerán'} del directorio y ${count === 1 ? 'su slug quedará disponible' : 'sus slugs quedarán disponibles'} nuevamente.${published ? ` ${published === 1 ? '1 está publicado y dejará de verse en el sitio' : `${published} están publicados y dejarán de verse en el sitio`}.` : ''}`,
    bulkDeleteAction: count => `Eliminar ${count}`,
    bulkDeleted: count => `${count} ${count === 1 ? 'recurso eliminado' : 'recursos eliminados'}.`,
    bulkDeletePartial: (success, failed) => `${success} eliminados; ${failed} no se pudieron eliminar. Siguen seleccionados.`, bulkCategoryChanged: count => `Se cambió la categoría de ${count} ${count === 1 ? 'recurso' : 'recursos'}.`, bulkExported: count => `Se exportaron ${count} ${count === 1 ? 'recurso' : 'recursos'}.`, bulkPartial: (success, failed) => `${success} completados; ${failed} no se pudieron actualizar.`,
  ...
  browseCsv: 'Elegir archivo',
  noCsvSelected: 'Ningún archivo seleccionado',
  ...
  hasFlags: 'Con reportes', flagged: 'Reportado', flaggedTooltip: count => `${count} ${count === 1 ? 'reporte abierto' : 'reportes abiertos'} — clic para revisar`,
  flagsListTitle: 'Reportes de este recurso', flagsListEmpty: 'Este recurso no tiene reportes.', flagsClose: 'Cerrar',
  flagStatusOpen: 'Abierto', flagStatusReviewing: 'En revisión', flagStatusResolved: 'Resuelto', flagStatusDismissed: 'Descartado',
  flagReasonLabel: key => ({ wrong_contact: 'Teléfono o dirección incorrectos', closed: 'Ya cerró', wrong_hours: 'Horario incorrecto', outdated: 'Información desactualizada', other: 'Otro' }[key] || key),
  flagReported: date => `Reportado el ${date}`, flagNote: 'Nota', flagArea: 'Código postal de origen', flagChangeStatus: 'Cambiar estado',
  flagStatusUpdated: 'Estado del reporte actualizado.', flagStatusUpdateError: 'No pudimos actualizar el estado del reporte.'
});
...
Object.assign(adminCopy.en, {
  ...
  bulkArchiveConfirm: count => `Archive ${count} ${count === 1 ? 'resource' : 'resources'}?`, bulkArchived: count => `${count} ${count === 1 ? 'resource was' : 'resources were'} archived.`,
    bulkDeleteConfirm: count => `Delete ${count} ${count === 1 ? 'resource' : 'resources'}?`,
    bulkDeleteDescription: (count, published) => `This action cannot be undone. ${count === 1 ? 'The resource will disappear' : 'The resources will disappear'} from the directory and ${count === 1 ? 'its slug will become' : 'their slugs will become'} available again.${published ? ` ${published === 1 ? '1 is published and will no longer appear on the site' : `${published} are published and will no longer appear on the site`}.` : ''}`,
    bulkDeleteAction: count => `Delete ${count}`,
    bulkDeleted: count => `${count} ${count === 1 ? 'resource was' : 'resources were'} deleted.`,
    bulkDeletePartial: (success, failed) => `${success} deleted; ${failed} could not be deleted. They remain selected.`, bulkCategoryChanged: count => `Category changed for ${count} ${count === 1 ? 'resource' : 'resources'}.`, bulkExported: count => `${count} ${count === 1 ? 'resource was' : 'resources were'} exported.`, bulkPartial: (success, failed) => `${success} completed; ${failed} could not be updated.`,
  ...
  browseCsv: 'Choose file',
  noCsvSelected: 'No file selected',
  ...
  hasFlags: 'Has reports', flagged: 'Flagged', flaggedTooltip: count => `${count} open ${count === 1 ? 'report' : 'reports'} — click to review`,
  flagsListTitle: 'Reports for this resource', flagsListEmpty: 'This resource has no reports.', flagsClose: 'Close',
  flagStatusOpen: 'Open', flagStatusReviewing: 'Reviewing', flagStatusResolved: 'Resolved', flagStatusDismissed: 'Dismissed',
  flagReasonLabel: key => ({ wrong_contact: 'Phone or address is wrong', closed: 'It has closed', wrong_hours: 'Wrong hours', outdated: 'Outdated information', other: 'Other' }[key] || key),
  flagReported: date => `Reported on ${date}`, flagNote: 'Note', flagArea: 'Origin ZIP code', flagChangeStatus: 'Change status',
  flagStatusUpdated: 'Report status updated.', flagStatusUpdateError: 'We could not update the report status.'
});
